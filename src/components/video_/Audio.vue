    ...
    <div id="controller">
      <v-layout pl-2>
         <v-flex xl1 sm1 md1>
          ...
          >
            <v-icon large color="#82B1FF" id="play_btn">play_circle_outline</v-icon>
          </v-btn>
        </v-flex>
        <v-flex xl8 sm8 md8>
          <v-slider
        ...
        <v-flex xl2 sm2 md2>
          <v-layout>
            <v-flex xl12 sm12 md12>
              ...
              >
                <v-icon large color="#82B1FF" id="audio_btn">volume_up</v-icon>
              </v-btn>
            </v-flex>
            <v-flex xl12 sm12 md12>
              <v-slider
...
export default {
  ...
  methods: {
    ...
    seek_timeupdate() {
      ...
      this.time = this.$T_change(Math.ceil(audio.currentTime)) + "/" + this.$T_change(Math.ceil(audio.duration));
    },
    seek_change() {
      if (this.slider === 0) {
