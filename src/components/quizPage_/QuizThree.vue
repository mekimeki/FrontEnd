    ...
    <v-dialog v-model="dialog" max-width="800">
        <v-tabs v-model="active" color="teal lighten-1" dark slider-color="yellow">
            ...
            <v-tab-item v-for="n in 7" :key="n">
                <v-card justify-center height="790">
                    <v-card v-if="n<7" flat>
                        <v-card-text v-if="n==1">
                            <div class="headName py-4 pl-5">퀴즈 푸는 방법</div>
                            <div class="mx-5 pb-3">
                                <v-img :src="require(`@/assets/Quiz/quiz3.png`)" style="height:470px" contain></v-img>
                            </div>
                            <div class="headName pt-5 px-5">{{ head }}</div>
                        </v-card-text>

                        <v-layout pb-3>
                            <v-flex xs3 sm12 flexbox>
                                <v-card-text v-if="n>=2">
                                    <div class="problem pt-3 pl-3">{{ (n-1) + text }}</div>
                                    <div class="pb-3 pt-4"></div>
                                    <div class="exampleUp3 mx-3 pt-5 mb-3">
                                        <v-img :src="image" style="height:200px" contain></v-img>
                                        <div class="pt-4 mt-3"></div>
                                        <span v-for="question in questions" :key="question">
                                        <span :v-on="box()" class="example px-4 pb-2 pt-5 mt-3" >{{ question }}</span>
                                        </span>
                                        <div class="pt-4"></div>
                                        <span class="example">{{ means }}</span>
                                    </div>
                                </v-card-text>
                            </v-flex>
                        </v-layout>
                            ...
                            <v-flex v-if="n>=2" xs3 sm12>
                                <v-layout justify-space-around row wrap>
                                    <v-flex v-for="n in 4" :key="n" md5>
                                        <v-btn large block color="teal lighten-3 title font-weight-bold" v-on:click="select=(n-1)" value="check">{{ n + ". " + example[n-1]}}</v-btn>
                                    </v-flex>
                                </v-layout>
                            </v-flex>
                        </v-layout>
...
export default {
    data() {
        return {
            active: null,
            dialog: false,
            head: "본 게임은 자신의 대사집에 저장되어 있는 대사의 스냅샷을 이용하여, 그림과 뜻에 맞는 빈칸을 채워넣는 게임입니다. 알맞은 답을 클릭해 주세요.",
            text: ". 알맞은 단어를 고르시오.",
            close: "닫기",
            end: "결과입니다",
            example: [],
    ...
    methods: {
        ...mapActions(['quiz_three_quest_actions', 'quest_post_actions']),
        box() {
            this.questions[this.blank] = '_______';
        },
        next() {
        ...
        getQuest() {
            ...
            this.quiz_three_quest_actions(this.ran_box).then(result => {
                const back = result;
                this.example = back.choice;
                this.questions = back.ques;
                this.answer = back.ans;
                this.blank = back.block;
                this.ran_box = back.ran_box;
                this.image = back.picture;
                this.means = back.explain;
